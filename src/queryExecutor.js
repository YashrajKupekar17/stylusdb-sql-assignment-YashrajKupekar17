...
function performLeftJoin(data, joinData, joinCondition, fields, table) {
    return data.flatMap(mainRow => {
        // Filter joinData to find matching rows based on the join condition
        ...
        // If no matching rows found, create a result row with null join data
        if (matchingJoinRows.length === 0) {
            return [createResultRow(mainRow, null, fields, table, true)];
        }

        // Map matching join rows to create result rows
        return matchingJoinRows.map(joinRow => createResultRow(mainRow, joinRow, fields, table, true));
...
function performInnerJoin(data, joinData, joinCondition, fields, table) {
    // Perform flatMap operation on the main dataset
    return data.flatMap(mainRow => {
        return joinData
            .filter(joinRow => {
                // Extract values from mainRow and joinRow based on joinCondition
                const mainValue = mainRow[joinCondition.left.split('.')[1]];
                const joinValue = joinRow[joinCondition.right.split('.')[1]];
                return mainValue === joinValue;
            })
            .map(joinRow => {
                // Map the fields to include in the resulting dataset
                return fields.reduce((acc, field) => {
                    const [tableName, fieldName] = field.split('.');
                    // Assign value based on the table name
...
function getValueFromRow(row, compoundFieldName) {
    // Split the compound field name into table name and field name
    const [tableName, fieldName] = compoundFieldName.split('.');
    // Access the field value from the row using the compound field name or just the field name
    return row[`${tableName}.${fieldName}`] || row[fieldName];
}
...
function performRightJoin(data, joinData, joinCondition, fields, table) {
    // Cache the structure of a main table row
...
function createResultRow(mainRow, joinRow, fields, table, includeAllMainFields) {
    // Initialize an empty object for the result row
    const resultRow = {};

    // Include all fields from the main table if specified
    if (includeAllMainFields) {
        Object.keys(mainRow || {}).forEach(key => {
            // Add main table fields with table prefix
            const prefixedKey = `${table}.${key}`;
            // Assign value from main row or null if main row is null
            resultRow[prefixedKey] = mainRow ? mainRow[key] : null;
        });
    }

    // Add or overwrite with the fields specified in the query
    fields.forEach(field => {
        // Extract table name and field name from the field
        const [tableName, fieldName] = field.includes('.') ? field.split('.') : [table, field];
        // Assign value based on the table name and availability of main and join rows
        resultRow[field] = tableName === table && mainRow ? mainRow[fieldName] : joinRow ? joinRow[fieldName] : null;
    });
                ...
                switch (aggFunc.toUpperCase()) {
                    ...
                    case 'MIN':
                        groupResults[groupKey].mins[aggField] = Math.min(
                            groupResults[groupKey].mins[aggField] || Infinity, // Changed value to Infinity
                            value
                        );
                        break;
                    case 'MAX':
                        groupResults[groupKey].maxes[aggField] = Math.max(
                            groupResults[groupKey].maxes[aggField] || -Infinity, // Changed value to -Infinity
                            value
                        );
                        break;
